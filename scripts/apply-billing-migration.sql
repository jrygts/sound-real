-- 🚨 CRITICAL BILLING FIX: Apply billing period tracking migration
-- Run this script in your Supabase SQL editor to add billing period tracking

-- Step 1: Add billing period tracking columns
...
-- Step 3: Update existing users with current billing period
UPDATE profiles 
SET 
...
-- Step 5: Record this migration (if schema_migrations table exists)
...
SELECT 
  ...
  COUNT(billing_period_start) as profiles_with_billing_start,
  COUNT(billing_period_end) as profiles_with_billing_end
FROM profiles; 