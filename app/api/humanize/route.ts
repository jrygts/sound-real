...
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      ...
      if (!isAdmin) {
        ...
        try {
          ...
          const bypassPlanCheck = isDevelopment && process.env.BYPASS_PLAN_CHECK === 'true';

          // Check if user has access to the service
          if (!bypassPlanCheck && !hasAccess) {
            return NextResponse.json(
          ...
          // Allow free tier users to process up to 1000 words per day
          const isFreeTier = profile?.plan_type === 'free';
          const freeTierDailyLimit = 1000;
          ...
          if (isFreeTier) {
            ...
            const dailyWordCount = dailyUsage?.reduce((sum, t) => sum + t.word_count, 0) || 0;
            
            if (dailyWordCount + wordsToProcess > freeTierDailyLimit) {
              return NextResponse.json(
                { 
                  error: "FREE_TIER_LIMIT_REACHED",
                  message: `Free tier daily limit (${freeTierDailyLimit} words) reached. Please upgrade for unlimited access.`,
                  upgradeUrl: "/pricing"
                },
                { status: 429 }
              );
            }
          } else if (wordsUsed + wordsToProcess > wordsLimit && wordsLimit !== -1) {
            return NextResponse.json(
    ...
    // Check FastAPI health before processing
    const isHealthy = await checkHumanizerHealth();
    if (!isHealthy) {
      console.error('🚨 [Transform] FastAPI is not healthy, falling back to original text');
      return NextResponse.json(
    ...
    // Use FastAPI for humanization with high aggressiveness for maximum modification
    const result = await humanizeText(cleanedText, {
      aggressiveness: 0.9  // 90% aggressiveness for aggressive mode
    });

    if (!result.success) {
      console.error('🚨 [Transform] FastAPI humanization failed:', result.error);
      return NextResponse.json(
        { error: result.error || "Failed to humanize text. Please try again." },
        { status: 500 }
      );
    }
    ...
    // Calculate realistic AI detection scores based on change percentage
    // Higher change percentage = lower AI detection score
    const changePercentage = result.changePercentage || 0;
    const mockScoreBefore = 0.87; // Mock: 87% AI-generated before
    const mockScoreAfter = Math.max(0.15, 0.85 - (changePercentage / 100 * 0.7)); // Lower score with more changes
    
    // console.log removed for prod (`✅ [Transform] FastAPI humanization completed successfully, ${changePercentage}% modified`);
